...
class Trainer:
    def __init__(self):
        ...
        self.training_dataloaders = MultitaskDatasetDataLoader(self._opt, train_mode ='Train', transform = train_transforms)
        ...
        print("Training sets: {} images ({} images per task)".format(len(self.training_dataloaders) * actual_bs,
    ...
    def _train(self):
        ...
        for i_student in range(self._opt.n_students):
            ...
            for i_epoch in range(1, self._opt.student_nepochs + 1):
                ...
                time_epoch = time.time() - epoch_start_time
                print('End of epoch %d / %d \t Time Taken: %d sec (%d min or %d h)' %
                      (i_epoch, self._opt.student_nepochs , time_epoch, time_epoch / 60, time_epoch / 3600))
    ...
    def _display_terminal(self, iter_start_time, i_epoch, i_train_batch, num_batches):
        ...
        output = "Time {}\tBatch Time {:.2f}\t Epoch [{}]([{}/{}])\t".format(
                                        start_time, t, 
    ...
    def _test(self):
        ...
        for task in self._opt.tasks:
            ...
            eval_items, eval_res = metric_func(preds, labels)
            now_time = time.strftime("%H:%M", time.localtime(val_start_time))
            output = "{} Test: Epoch [{}] Step [{}] loss {:.4f} Eval_0 {:.4f} Eval_1 {:.4f}".format(
                task, now_time, self._total_steps, val_errors['loss'], eval_items[0], eval_items[1])
            print(output)
            if task != 'VA':
                eval_per_task[task] = eval_res
            else:
