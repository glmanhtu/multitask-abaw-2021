...
class DatasetMixedEXPR(DatasetBase):
    ...
    def __getitem__(self, index):
        ...
        for i, row in df.iterrows():
            ...
            label = row['label']
            va.append([row['valence'], row['arousal']])
            images.append(image)
            labels.append(label)
        ...
        sample = {'image': torch.stack(images, dim=0),
                  'label': np.array(labels).astype(np.long),
                  'sub': np.array(va).astype(np.float32),
                  'path': img_paths,
                  'index': index
                  }
    ...
    def _read_audio(self, seq):
        ...
        spr = self.sample_rate
        numb_frames = int(timestamps[to_frame] * spr) - int(timestamps[from_frame] * spr)
        frame_offset = int(timestamps[from_frame] * spr)
        seq_timestamps = timestamps[from_frame:to_frame + 1]
        waveform, sample_rate = torchaudio.load(meta_data['audio_file'], frame_offset=frame_offset, num_frames=numb_frames)
        waveform = self.audio_transform(waveform)
        waveform = self.audio_spec_transform(waveform)

        # Compute the position of video frames corresponding to the audio frames
